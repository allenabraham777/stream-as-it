...
type thumbProps = {
  videoRef: any;
  bridgeSetter: (videoRef: any, keyName: string) => void;
  keyName: string;
  id: string;
  status: boolean;
  label: string;
    ...
    <div className="h-[100%] border border-gray-500 rounded-md overflow-hidden relative group">
      ...
      {!disableController && (
        <div className="absolute z-20 right-2 top-2 bg-gray-50 border text-gray-900 flex rounded-sm font-semibold">
          ...
          <button
            className={`p-1 ${
              controllerOptions.video && "bg-blue-700 text-white"
            }`}
...
const VideoList = (props: Props) => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.srcObject = videoSettings.video
        ? videoSettings.stream
        : null;
  ...
  useEffect(() => {
    if (screenRef.current) {
      screenRef.current.srcObject = videoSettings.screen
        ? videoSettings.screenStream
        : null;
    }
  }, [screenRef, videoSettings]);

  const streamControl = (videoRef: any, keyName: string) => {

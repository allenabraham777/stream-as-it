import { fitImage } from "helpers/canvas";
...
export const drawUserVideo = (
  ...
  let videoElement: HTMLVideoElement | HTMLImageElement =
    document.getElementById("user-video") as HTMLVideoElement;

  if (!video) return;

  let maxPrintWidth = (7 * canvasWidth) / 8;
  let maxPrintHeight = (7 * canvasHeight) / 8;
  let x = (1 * canvasWidth) / 16;
  ...
  const labelHeight =
    screen && isScreenActive ? 0.1 * maxPrintHeight : 0.04 * maxPrintHeight;
  drawUserLabel(
    context,
    "Allen",
    brand,
    x,
    y + maxPrintHeight - labelHeight,
    labelHeight
  );
};

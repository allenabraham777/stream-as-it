...
export class BaseStreamGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      console.error(
        `Error creating connection with ${client.id}. Reason: ${error.message}`,
      );
  ...
  protected startStream(client: Socket, type: string) {
    const ffmpegProcess: ffmpeg.FfmpegCommand =
      this.ffmpegProcesses[type][client.id];
    ffmpegProcess.run();
  }
  ...
  private setFFmpegProcess(
    ...
    const ffmpegProcess = ffmpeg()
      ...
      .on('start', () =>
        console.log(
          `FFmpeg process started for type: ${type} under client: ${clientId}`,
        ),
      )
      .on('error', (err) =>
        console.error(
          'FFmpeg error for type: ${type} under client: ${clientId}:',
          err,
        ),
      )
      .on('end', () =>
        console.log(
          'FFmpeg process ended for type: ${type} under client: ${clientId}',
        ),
      );
